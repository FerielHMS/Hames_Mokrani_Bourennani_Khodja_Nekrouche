...
async function loadActivityLog() {
    ...
    if (filtered.length === 0) { 
        tbody.innerHTML = '<tr><td colspan="6">NO WORKER ACTIVITIES FOUND<\/tr>'; 
        return; 
    }
    var html = '';
...
async function loadAdminData() {
    ...
    if (cars.length === 0) { 
        carsList.innerHTML = '<tr><td colspan="8">NO CARS IN DATABASE. ADD SOME!<\/tr>'; 
    } else { 
        var carsHtml = '';
    ...
    if (bookings.length === 0) { 
        bookingsList.innerHTML = '<tr><td colspan="9">NO BOOKINGS YET<\/tr>'; 
    } else { 
        var bookingsHtml = '';
        for (var i = 0; i < bookings.length; i++) {
            var b = bookings[i];
    ...
    if (orders.length === 0) { 
        ordersList.innerHTML = '<tr><td colspan="7">NO ORDERS YET<\/tr>'; 
    } else { 
        var ordersHtml = '';
        for (var i = 0; i < orders.length; i++) {
            var o = orders[i];
    ...
    if (users.length === 0) { 
        usersList.innerHTML = '<tr><td colspan="4">NO REGISTERED USERS<\/tr>'; 
    } else { 
        var usersHtml = '';
